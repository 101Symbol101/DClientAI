...
SysGetIPAddresses() {
    ...
    try {
        ...
        exec := shell.Exec("ipconfig")
        output := exec.StdOut.ReadAll()
        lines := StrSplit(output, "`n")
        for i, line in lines {
            line := Trim(line)
            if RegExMatch(line, "i)(IPv4|IP)\s+Address[^:]*:\s*(\d+\.\d+\.\d+\.\d+)", &match) {
                ip := match[2]
            } else if RegExMatch(line, ":\s*(\d+\.\d+\.\d+\.\d+)(?!\d)", &match2) {
                ip := match2[1]
            } else {
                continue
            }
            
            if (ip != "127.0.0.1" && ip != "0.0.0.0" && !InStr(ip, "127.") && !InStr(ip, "169.254.")) {
                found := false
                for j, addr in addresses {
                    if addr = ip {
                        found := true
                        break
                    }
        ...
        try {
            ...
            if ws2_32 {
                wsaData := Buffer(400)
                DllCall("ws2_32\WSAStartup", "UShort", 0x0202, "Ptr", wsaData.Ptr, "Int")
                sock := DllCall("ws2_32\socket", "Int", 2, "Int", 2, "Int", 17, "UInt")
                if sock != 0xFFFFFFFF {
                    addr := Buffer(16, 0)
                    NumPut("UShort", 2, addr, 0)
                    NumPut("UShort", DllCall("ws2_32\htons", "UShort", 53, "UShort"), addr, 2)
                    NumPut("UInt", DllCall("ws2_32\inet_addr", "Str", "8.8.8.8", "UInt"), addr, 4)
                    
                    DllCall("ws2_32\connect", "UInt", sock, "Ptr", addr.Ptr, "Int", 16, "Int")
                    
                    localAddr := Buffer(16)
                    addrLen := Buffer(4)
                    NumPut("Int", 16, addrLen)
                    if DllCall("ws2_32\getsockname", "UInt", sock, "Ptr", localAddr.Ptr, "Ptr", addrLen.Ptr, "Int") = 0 {
                        ip := NumGet(localAddr, 4, "UInt")
                        ipStrPtr := DllCall("ws2_32\inet_ntoa", "UInt", ip, "Ptr")
                        if ipStrPtr {
                            ipAddress := StrGet(ipStrPtr, "UTF-8")
                            if (ipAddress != "" && !InStr(ipAddress, "127.") && ipAddress != "0.0.0.0") {
                                addresses.Push(ipAddress)
                            }
                        }
                    }
                    
                    DllCall("ws2_32\closesocket", "UInt", sock, "Int")
                }
