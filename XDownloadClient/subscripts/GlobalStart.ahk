...
; Returns color code based on log message content
GetLogColor(message) {
...
; Builds RTF header with color table for RichEdit control
GetRTFHeader() {
    bs := Chr(92)
...
; Callback for streaming RTF data to RichEdit control
...
; Sets RTF content in RichEdit control via stream
...
; Formats log entry as RTF with color coding
...
; Adds log message to history and file, updates UI
...
; Loads log history from file (newest first)
LoadLogsFromFile() {
...
; Initialize GUI and UI variables
try {
...
; Create main server control GUI
try {
...
try {
    ...
    startupErrors.Push("Failed to load settings: " . (HasProp(err, "Message") ? err.Message : String(err)))
}
Sleep(100)
...
if (serverGuiX != "" && serverGuiY != "" && serverGuiW != "" && serverGuiH != "") {
    ...
    serverGui.Show("w" . guiWidth . " h" . (guiHeight - 50))
}
Sleep(100)
...
try {
    ...
    startupErrors.Push("Failed to apply dark theme: " . (HasProp(err, "Message") ? err.Message : String(err)))
}
Sleep(50)
...
try {
    ...
    startupErrors.Push("Failed to apply window skin: " . (HasProp(err, "Message") ? err.Message : String(err)))
}
Sleep(100)
...
serverGui.OnEvent("Close", (*) => (SetTimer(SaveServerGuiPosition, 0), AddLogMessage("=== Application Shutting Down ==="), Sleep(100), SaveServerGuiPosition(), Sleep(50), AddLogMessage("Saving settings..."), SaveSettings(), Sleep(50), AddLogMessage("Cleaning up resources..."), Sleep(100), AddLogMessage("Application closed"), ExitApp()))
Sleep(50)
...
StartServer(*) {
    ...
    try {
        ...
        statusText.Opt("cRed")
        AddLogMessage("Failed to write HTML file: " . err.Message)
        return
    }
...
; Opens server URL in default browser
OpenBrowser(*) {
...
; Opens/closes log viewer window
OpenLogGui(*) {
...
; Clears all logs from memory and file
DeleteLogs(*) {
...
SaveSettings() {
    ...
    try {
        ...
        AddLogMessage("Error saving settings: " . (HasProp(err, "Message") ? err.Message : String(err)))
    }
}
...
; Applies window skin using USkin library
ApplySkin(hWnd := 0) {
