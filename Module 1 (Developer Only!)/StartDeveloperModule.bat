...
if exist "..\Module 13 (Webserve)\subscripts\AutoHotkey64.exe" (
    start "" "..\Module 13 (Webserve)\subscripts\AutoHotkey64.exe" "DeveloperModule.ahk"
) else if exist "..\Module 13 (Webserve)\subscripts\AutoHotkey32.exe" (
    start "" "..\Module 13 (Webserve)\subscripts\AutoHotkey32.exe" "DeveloperModule.ahk"
) else (
    REM Try to use system AutoHotkey if installed
    where autohotkey.exe >nul 2>&1
    if %errorlevel% equ 0 (
        start "" autohotkey.exe "DeveloperModule.ahk"
    ) else (
